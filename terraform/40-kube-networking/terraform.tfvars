...
# Secrets (public-domain, acme-email) live in Azure Key Vault, read via
# data.azurerm_key_vault_secret in main.tf. The Cloudflare DNS token is not
# among them: cert-manager receives it through ESO (cert-manager.tf).
#
# Every chart's values live in values/<chart>.yaml, which CI renders against
# the pinned version before merge -- so a bump that invalidates a key fails on
# the PR rather than at apply.
...
# longhorn_v2_data_engine is not set here: it is owned by 30-talos, which
# reserves the hugepages the engine needs, and read from that layer's state.
...
traefik_crds_chart_version = "1.18.0"  # renovate: helmRepo=https://traefik.github.io/charts chart=traefik-crds
